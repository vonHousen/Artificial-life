...
public:
	...
	/**
	 * Getter for location of nearest Herbivore edible for Carnivore.
	 * @param hungryCarnivore - Organism for which nearest location is calculated
	 * @return Vector pointing to location of nearest food / Vector of length zero if not found.
	 */
	Vector getVectorToNearestPrey(Carnivore* hungryCarnivore) const;
	 ...
	 */
	Organism* getOrganismAt(const Vector& location);
